...
[Verb("coding-guidelines-create")]
public class CodingGuidelinesCreateRequest : IRequest
{
    [Option('d', "directory", Required = false, HelpText = "The directory where the coding guidelines file will be created.")]
    public string Directory { get; set; } = Environment.CurrentDirectory;
}

public class CodingGuidelinesCreateRequestHandler : IRequestHandler<CodingGuidelinesCreateRequest>
{
    private readonly ILogger<CodingGuidelinesCreateRequestHandler> logger;
    private readonly IArtifactGenerator artifactGenerator;
    private readonly IFileSystem fileSystem;

    public CodingGuidelinesCreateRequestHandler(
        ILogger<CodingGuidelinesCreateRequestHandler> logger,
    ...
    public async Task Handle(CodingGuidelinesCreateRequest request, CancellationToken cancellationToken)
    {
        logger.LogInformation("Creating coding guidelines file.");

        var docsDirectory = Path.Combine(request.Directory, "docs");

        var content = GetCodingGuidelinesContent();

        var model = new ContentFileModel(content, "coding-guidelines", docsDirectory, ".md");

        await artifactGenerator.GenerateAsync(model);

        logger.LogInformation("Coding guidelines file created at {Path}", model.Path);
    }

    private static string GetCodingGuidelinesContent()
    {
        return """
