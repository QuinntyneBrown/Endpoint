...
{
    private readonly ILogger<BootstrapAddRequestHandler> logger;
    ...
    {
        logger.LogInformation("Handled: {0}", nameof(BootstrapAddRequestHandler));

        await angularService.BootstrapAdd(new AngularProjectReferenceModel(request.Name, request.Directory));
    }
}
